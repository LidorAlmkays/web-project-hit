        ...
        if (response != null && response.getData() != null) {
            ...
            if (responsePacket != null && responsePacket.getMessage() != null) {
                ...
                if (rawMsg.trim().startsWith("[")) {
                    ...
                    // Legacy fallback: Server returned valid email string (old version)
                    // Create a dummy info object
                    requests = new java.util.ArrayList<>();
                    requests.add(new PendingRequestInfo(rawMsg, System.currentTimeMillis())); // Reset time as unknown
                }
            }
        }
